...
const MacOSDock: React.FC<MacOSDockProps> = ({ 
  ...
  const getResponsiveConfig = useCallback(() => {
    ...
    if (isMobile) {
      // Mobile phones - optimized for professional look
      return {
      ...
      // Desktop and large screens - original professional size
      return {
  ...
  // Ensure the panel is tall enough for the icons and indicators
  const panelHeight = isMobileView ? baseIconSize * 1.15 : baseIconSize * 1.05;
  ...
  const padding = Math.max(8, baseIconSize * 0.12);
  const pillHeight = panelHeight + padding * 2;
  // Total container height should be enough for the maximum scaled icon
  const containerHeight = baseIconSize * maxScale + padding * 2;
  // Center icons vertically within the pill background
  const iconBottomOffset = (pillHeight - baseIconSize) / 2;
    ...
    <div 
      ...
      style={{
        ...
        paddingBottom: `${iconBottomOffset}px`,
        // No background here, it's moved to a separate div
      }}
      onMouseMove={handleMouseMove}
      onMouseLeave={handleMouseLeave}
    >
      {/* The Dock Background Pill */}
      <div 
          ...
          return (
            <div
              ...
              style={{
                ...
                height: `${baseIconSize}px`,
                transform: `translate3d(${padding + position - baseIconSize / 2}px, 0, 0) scale(${scale})`,
                transformOrigin: 'bottom center',
