...
const ThemeToggle = () => {
  ...
  return (
    <div className="flex items-center justify-center">
      <div className="flex items-center rounded-full border border-dotted border-gray-600 p-1">
        <button
          onClick={toggleTheme}
          className="bg-black mr-3 rounded-full p-2 text-white dark:bg-gray-900 dark:text-white cursor-pointer"
          title="Toggle Light Mode"
        >
          <Sun className="h-4 w-4 stroke-1" />
          <span className="sr-only">Toggle Theme</span>
        </button>

        <button type="button" onClick={handleScrollTop} className="text-gray-400 hover:text-white transition-colors cursor-pointer">
        ...
        <button
          onClick={toggleTheme}
          className="dark:bg-black ml-3 rounded-full p-2 text-black bg-gray-200 dark:text-white hover:bg-gray-300 dark:hover:bg-gray-800 transition-colors cursor-pointer"
          title="Toggle Dark Mode"
        >
          <Moon className="h-4 w-4 stroke-1" />
          <span className="sr-only">Toggle Theme</span>
        </button>
      </div>
    </div>
...
export default function Footer() {
      ...
      sections: [
        {
          ...
          items: [
            ...
            { name: t('footer.careers'), href: "/career" },
        ...
        },
        {
          ...
          items: [
            { name: t('footer.studentDev'), href: "/student-page" },
        ...
        {
          id: "office",
          name: t('footer.office'),
          items: [
            { name: t('footer.addressLine1'), href: "#" },
            { name: t('footer.addressLine2'), href: "#" },
            { name: t('footer.addressLine3'), href: "#" },
            { name: t('footer.addressLine4'), href: "#" },
          ],
        }
      ],
    },
  ...
  const Underline = `hover:-translate-y-1 border border-dotted border-gray-600 rounded-xl p-2.5 transition-transform ${
    theme === 'dark' ? 'text-gray-400 hover:text-white hover:border-white' : 'text-gray-600 hover:text-black hover:border-black'
  }`;

  return (
    <footer 
      className={`border-t px-2 transition-colors duration-300 ${
        theme === 'dark' ? 'border-gray-800 bg-black text-white' : 'border-gray-200 bg-gray-50 text-black'
      }`} 
      dir={dir}
    >
      {/* Welcome Block */}
      <div className="relative mx-auto grid max-w-7xl items-center justify-center gap-6 p-10 pb-0 md:flex flex-col md:flex-row">
        <div className="flex items-center justify-center rounded-full cursor-pointer" onClick={() => navigate('/')}>
          <img src="/logo.png" alt="Blunet" className="h-12 md:h-16 object-contain" />
        </div>
        <p className={`bg-transparent text-center text-sm leading-relaxed md:text-left max-w-3xl font-sora ${
      ...
      {/* Headquarters and Contact Section (Matching attached mockup layout) */}
      <div id="contact" className="mx-auto max-w-7xl px-6 pt-10 pb-8 w-full">
        <div className={`rounded-3xl p-6 sm:p-10 border grid md:grid-cols-2 gap-10 items-stretch transition-colors duration-300 ${
          theme === 'dark' 
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        <div className={`border-b border-dotted ${theme === 'dark' ? 'border-gray-800' : 'border-gray-300'}`}> </div>
        <div className="py-10 flex justify-center md:justify-start">
          {categories.map((category) => (
            <div
              key={category.id}
              className="grid grid-cols-1 sm:grid-cols-4 flex-row justify-between gap-12 sm:gap-24 leading-6 md:flex w-full"
            >
              {category.sections.map((section) => (
                <div key={section.id} className={dir === 'rtl' ? 'text-right' : 'text-left'}>
                  <h3 className={`font-semibold mb-4 text-base tracking-wider font-poppins uppercase ${
                    ...
                    {section.items.map((item, idx) => (
                      <li key={idx} className="flow-root">
                        {item.href.startsWith('/') ? (
                          <Link 
                            to={item.href}
                            className={`text-sm transition-colors font-sora ${
                          ...
                          <span className="text-sm text-gray-500 cursor-default font-sora">
                            {item.name}
                          </span>
                        )}
                      </li>
      ...
      <div className="flex flex-wrap justify-center gap-y-6">
        <div className="flex flex-wrap items-center justify-center gap-6 gap-y-4 px-6">
          <div className={Underline.replace("hover:text-white hover:border-white hover:-translate-y-1", "cursor-default").replace("hover:text-black hover:border-black hover:-translate-y-1", "cursor-default")}>
      ...
      <div className="mx-auto mb-10 mt-10 flex flex-col justify-between text-center text-sm md:max-w-7xl font-mono">
        <div className="flex flex-row items-center justify-center gap-1.5 text-gray-500">
          <span> © </span>
